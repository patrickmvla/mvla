...
const TRACKED_REPOS: Record<
  string,
  { status: "active" | "shipped"; href: string; description: string }
> = {
  hachi: {
    status: "active",
    href: "https://hachii.vercel.app/",
    description: "visual platform for building RAG systems",
  },
  stableflow: {
    status: "active",
    href: "https://github.com/patrickmvla/stableflow",
    description: "workflow orchestration engine",
  },
  "permissions-engine": {
    status: "active",
    href: "https://github.com/patrickmvla/permissions-engine",
    description: "fine-grained authorization system",
  },
  "payment-engine": {
    status: "shipped",
    href: "https://github.com/patrickmvla/payment-engine",
    description: "transaction processing backbone",
  },
};
...
export const github = new Hono()
  .get("/activity", async (c) => {
    try {
      ...
      if (!eventsRes.ok) {
        return c.json(
          { error: "github api error", status: eventsRes.status },
          502
        );
      }
  ...
  .get("/stats", async (c) => {
    try {
      ...
      if (!res.ok) {
        return c.json(
          { error: "github graphql error", status: res.status },
          502
        );
      }
      ...
      const commitsThisWeek = currentWeek.reduce(
        (sum: number, d: { contributionCount: number }) =>
